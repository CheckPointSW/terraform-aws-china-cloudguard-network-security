...
module "example_module" {
    ...
    source  = "CheckPointSW/china-cloudguard-network-security/aws//modules/cme_iam_role"
    version = "1.0.0"
...
| external_id     | External ID required for cross-account role assumption (confused deputy protection). Mandatory when `trusted_account` is set; ignored when `trusted_account` is empty.   | string       | **Default:** "" (empty)                                                                                                                      |
