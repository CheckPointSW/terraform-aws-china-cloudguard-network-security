...
resource "aws_vpc_security_group_ingress_rule" "ingress_rule_ipv4_custom" {
  for_each = { for idx, rule in var.security_rules : idx => rule if rule.direction == "ingress" }
...
resource "aws_vpc_security_group_ingress_rule" "ingress_rule_ipv4_default" {
  count = length([for rule in var.security_rules : rule if rule.direction == "ingress"]) == 0 ? 1 : 0
...
resource "aws_vpc_security_group_egress_rule" "egress_rule_ipv4_custom" {
  for_each = { for idx, rule in var.security_rules : idx => rule if rule.direction == "egress" }
...
resource "aws_vpc_security_group_egress_rule" "egress_rule_ipv4_default" {
  count = length([for rule in var.security_rules : rule if rule.direction == "egress"]) == 0 ? 1 : 0
